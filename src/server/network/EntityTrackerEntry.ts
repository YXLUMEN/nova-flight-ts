...
export class EntityTrackerEntry {
    ...
    public constructor(world: ServerWorld, entity: Entity, tickInterval: number, trackDelta: boolean) {
        ...
        this.posDelta.setBase(entity.positionRef.x, entity.positionRef.y);
    ...
    public tick() {
        if (this.trackingTick % this.tickInterval === 0 || this.entity.needSync || this.entity.getDataTracker().isDirty()) {
            ...
            // 同步位置
            const dx = this.posDelta.encodeX(entityPos);
            const dy = this.posDelta.encodeY(entityPos);
            ...
            this.syncEntityData();
            if (syncPos) {
                this.posDelta.setBase(entityPos.x, entityPos.y);
            }
