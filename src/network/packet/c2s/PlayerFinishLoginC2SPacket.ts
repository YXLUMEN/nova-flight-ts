...
export class PlayerFinishLoginC2SPacket implements Payload {
    ...
    public static readonly CODEC: PacketCodec<PlayerFinishLoginC2SPacket> = PacketCodecs.of<PlayerFinishLoginC2SPacket>(
        (writer, value) => {
            writer.writeUUID(value.uuid);
        },
        (reader) => {
            return new PlayerFinishLoginC2SPacket(reader.readUUID());
        }
    );
