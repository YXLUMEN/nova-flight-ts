...
export class HistoricalScoreRender implements StatisticItem {
    ...
    public async render() {
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            box.append(this.createItem(records[i], i));
        }
        return box;
    }
    ...
    private createItem(record: ScoreRecord, index: number): HTMLElement {
        ...
        this.appendSubitem(root, '击杀效率', record.killEffective.toString());
        this.appendSubitem(root, '存活时长', record.totalSurvivalTime.toString());
        this.appendSubitem(root, '玩家名称', record.playerName);
        this.appendSubitem(root, '存档名称', record.worldName);
        this.appendSubitem(root, '游戏版本', record.version ?? '未知版本');
        this.appendSubitem(root, '记录时间', record.recordTime);
        if (record.devMode) {
