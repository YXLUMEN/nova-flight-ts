...
export class InventoryRender implements IUi {
    ...
    public render(ctx: CanvasRenderingContext2D): void {
        ...
        const specialLen = inventory.specialLength();
    ...
    private renderItem(ctx: CanvasRenderingContext2D, x: number, y: number, item: ItemStack): void {
        const texture = TextureManager.getModel(item.getItem().getRegistryEntry());
        ctx.drawImage(texture, x + 4, y + 4, this.ICON_SIZE, this.ICON_SIZE);
    }
    ...
    public setSize(w: number, h: number): void {
        ...
        const inventory = this.inventory.getInventory();
        this.recalculateLayout(inventory.hotbarLength(), inventory.specialLength(), inventory.maxSize());
    }
